...
class CampusNotifier
  ...
  final Map<int, List<CampusResponse>> cachedPages =
      <int, List<CampusResponse>>{};
  int lastPage = 0;
  static const int _pageSize = 6;

  CampusNotifier(this.repository, this.restaurantId)
      : super(const AsyncLoading<BaseResponse<List<CampusResponse>>>());

  Future<void> loadData() async {
    if (cachedPages.isNotEmpty) {
      final List<CampusResponse> allCachedData = cachedPages.values
          .expand((List<CampusResponse> page) => page)
          .toList();
      state = AsyncData<BaseResponse<List<CampusResponse>>>(
          BaseResponse<List<CampusResponse>>(
              code: "SUCCESS", data: allCachedData));
      return;
    }
    try {
      await fetchData(0, _pageSize);
    } on Exception catch (e, stack) {
      state = AsyncError<BaseResponse<List<CampusResponse>>>(e, stack);
    }
  }
